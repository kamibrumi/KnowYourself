...
public class GradientTreeBoost implements SoftClassifier<double[]>, Serializable {
    ...
    public GradientTreeBoost(Attribute[] attributes, double[][] x, int[] y, int ntrees, int maxNodes, double shrinkage, double subsample) {
        ...
        this.shrinkage = shrinkage;
        this.subsample = subsample;
        this.k = smile.math.Math.max(y) + 1;
    ...
    private void train2(Attribute[] attributes, double[][] x, int[] y) {        
        ...
        double mu = smile.math.Math.mean(y2);
        b = 0.5 * smile.math.Math.log((1 + mu) / (1 - mu));
        ...
        for (int m = 0; m < ntrees; m++) {
            ...
            smile.math.Math.permutate(perm);
            for (int l = 0; l < k; l++) {
                int subj = (int) smile.math.Math.round(nc[l] * subsample);
                int count = 0;
            ...
            for (int i = 0; i < n; i++) {
                response[i] = 2.0 * y2[i] / (1 + smile.math.Math.exp(2 * y2[i] * h[i]));
            }
    ...
    private void traink(Attribute[] attributes, double[][] x, int[] y) {        
        ...
        for (int m = 0; m < ntrees; m++) {
            for (int i = 0; i < n; i++) {
                ...
                double Z = 0.0;
                for (int j = 0; j < k; j++) {
                    p[j][i] = smile.math.Math.exp(h[j][i] - max);
                    Z += p[j][i];
                }
    ...
    class L2NodeOutput implements RegressionTree.NodeOutput {
        ...
        public double calculate(int[] samples) {
            ...
            for (int i = 0; i < samples.length; i++) {
                if (samples[i] > 0) {
                    double abs = smile.math.Math.abs(y[i]);
                    nu += y[i];
                    de += abs * (2.0 - abs);
                }
            }
    ...
    public int predict(double[] x, double[] posteriori) {
        ...
        if (k == 2) {
            ...
            posteriori[0] = 1.0 / (1.0 + smile.math.Math.exp(2*y));
            ...
            double Z = 0.0;
            for (int i = 0; i < k; i++) {
                posteriori[i] = smile.math.Math.exp(posteriori[i] - max);
                Z += posteriori[i];
            }
        ...
        if (k == 2) {
            ...
            for (int i = 0; i < ntrees; i++) {
                for (int j = 0; j < n; j++) {
                    ...
                    label[j] = smile.math.Math.whichMax(prediction[j]);
                }
