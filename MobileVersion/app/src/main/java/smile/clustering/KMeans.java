...
import smile.ClusteringDistance;
import smile.PartitionClustering;
...
public class KMeans extends PartitionClustering<double[]> implements Serializable {
    ...
    public int predict(double[] x) {
        ...
        for (int i = 0; i < k; i++) {
            double dist = smile.math.Math.squaredDistance(x, centroids[i]);
            if (dist < minDist) {
                minDist = dist;
                bestCluster = i;
            }
        ...
        for (int iter = 0; iter < maxIter; iter++) {
            ...
            if (Double.isNaN(wcss)) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    for (int j = 0; j < k; j++) {
                        double dist = smile.math.Math.squaredDistance(data[i], centroids[j]);
                        if (nearest > dist) {
                            y[i] = j;
                            nearest = dist;
                        }
