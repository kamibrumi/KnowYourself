...
public abstract class PartitionClustering <T> implements Clustering<T> {
    ...
    public static int[] seed(double[][] data, int k, ClusteringDistance distance) {
        int n = data.length;
        int[] y = new int[n];
        double[] centroid = data[smile.math.Math.randomInt(n)];
        ...
        for (int j = 1; j < k; j++) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                switch (distance) {
                    case EUCLIDEAN:
                        dist = smile.math.Math.squaredDistance(data[i], centroid);
                        break;
                    case EUCLIDEAN_MISSING_VALUES:
                        dist = squaredDistance(data[i], centroid);
                        break;
                    case JENSEN_SHANNON_DIVERGENCE:
                        dist = smile.math.Math.JensenShannonDivergence(data[i], centroid);
                        break;
                }
            ...
            double cutoff = smile.math.Math.random() * smile.math.Math.sum(d);
            double cost = 0.0;
            int index = 0;
            for (; index < n; index++) {
                cost += d[index];
    ...
    public static <T> double seed(smile.math.distance.Distance<T> distance, T[] data, T[] medoids, int[] y, double[] d) {
        ...
        T medoid = data[smile.math.Math.randomInt(n)];
        medoids[0] = medoid;
