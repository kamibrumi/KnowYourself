...
public class DecisionTree implements SoftClassifier<double[]>, Serializable {
    ...
    class TrainNode implements Comparable<TrainNode> {
        ...
        public int compareTo(TrainNode a) {
            return (int) smile.math.Math.signum(a.node.splitScore - node.splitScore);
        }
        ...
        public boolean findBestSplit() {
            ...
            if (mtry < p) {
                smile.math.Math.permutate(variables);
        ...
        public Node findBestSplit(int n, int[] count, int[] falseCount, double impurity, int j) {
            ...
            if (attributes[j].getType() == Attribute.Type.NOMINAL) {
                ...
                for (int l = 0; l < m; l++) {
                    int tc = smile.math.Math.sum(trueCount[l]);
                    int fc = n - tc;
                    ...
                    int trueLabel = smile.math.Math.whichMax(trueCount[l]);
                    int falseLabel = smile.math.Math.whichMax(falseCount);
                ...
                for (int i : order[j]) {
                    if (samples[i] > 0) {
                        ...
                        int tc = smile.math.Math.sum(trueCount);
                        int fc = n - tc;
                        ...
                        int trueLabel = smile.math.Math.whichMax(trueCount);
                        int falseLabel = smile.math.Math.whichMax(falseCount);
        ...
        switch (rule) {
            ...
            case ENTROPY:
                for (int i = 0; i < count.length; i++) {
                    if (count[i] > 0) {
                        double p = (double) count[i] / n;
                        impurity -= p * smile.math.Math.log2(p);
                    }
                }
                break;
            case CLASSIFICATION_ERROR:
                ...
                for (int i = 0; i < count.length; i++) {
                    if (count[i] > 0) {
                        impurity = smile.math.Math.max(impurity, count[i] / (double)n);
                    }
                }
                impurity = smile.math.Math.abs(1 - impurity);
                break;
        }
    ...
    public DecisionTree(Attribute[] attributes, double[][] x, int[] y, int maxNodes, int nodeSize, int mtry, SplitRule rule, int[] samples, int[][] order) {
        ...
        root = new Node(smile.math.Math.whichMax(count), posteriori);
