...
public class CLARANS <T> extends PartitionClustering<T> implements Serializable {
    ...
    public CLARANS(T[] data, Distance<T> distance, int k) {
        this(data, distance, k, (int) smile.math.Math.round(0.0125 * k * (data.length - k)));
    }
    ...
    public CLARANS(T[] data, Distance<T> distance, int k, int maxNeighbor) {
        this(data, distance, k, maxNeighbor, smile.math.Math.max(2, MulticoreExecutor.getThreadPoolSize()));
    }
    ...
    private double getRandomNeighbor(T[] data, T[] medoids, int[] y, double[] d) {
        ...
        int index = smile.math.Math.randomInt(k);
        T medoid = null;
        boolean dup;
        do {
            dup = false;
            medoid = data[smile.math.Math.randomInt(n)];
            for (int i = 0; i < k; i++) {
                if (medoid == medoids[i]) {
                    dup = true;
                    break;
    ...
    public String toString() {
        ...
        for (int i = 0; i < k; i++) {
            int r = (int) smile.math.Math.round(1000.0 * size[i] / y.length);
