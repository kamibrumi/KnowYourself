...
public class RandomForest implements SoftClassifier<double[]>, Serializable {
    ...
    static class TrainingTask implements Callable<Tree> {
        ...
        public Tree call() {
            ...
            if (subsample == 1.0) {
                ...
                for (int l = 0; l < k; l++) {
                    int subj = (int) smile.math.Math.round(nc[l] * subsample / classWeight[l]);
                    int count = 0;
    ...
    public RandomForest(Attribute[] attributes, double[][] x, int[] y, int ntrees) {
        this(attributes, x, y, ntrees, (int) smile.math.Math.floor(smile.math.Math.sqrt(x[0].length)));
    }
    ...
    public int predict(double[] x) {
        ...
        return smile.math.Math.whichMax(y);
    }
